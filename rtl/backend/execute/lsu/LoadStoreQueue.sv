...
(
    ...
    integer queryOffset;
    integer queryIndex;
    integer recoveryOffset;
    integer recoveryIndex;
    logic queryFound;
                    ...
                    if ((queryOffset < entryCount) && !queryFound) begin
                        if (queryIndex == integer'(issueTag_i)) begin
                            ...
                            if (!entries[queryIndex].addressReady) begin
                                issueReady_o = 1'b0;
                            end else if ((entries[queryIndex].address >=
                                         MMIO_BASE_ADDR) &&
                                        (entries[queryIndex].address <=
                                         MMIO_LAST_ADDR)) begin
                                // MMIO Stores are ordering barriers. No
                                // younger Load may pass their externally
                                // visible side effect, even to another word.
                                issueReady_o = 1'b0;
    ...
    // Recovery compaction is independent of the normal head/query datapath.
    // Keep the original mask/count semantics while exposing that independence
    // to synthesis and lint, so branch recovery does not appear to feed LSQ
    // issue Ready combinationally.
    always_comb begin
        recoveryRetainedCount = 0;
        recoveryIndex = 0;
        for (recoveryOffset = 0; recoveryOffset < DEPTH;
             recoveryOffset = recoveryOffset + 1) begin
            recoveryIndex = integer'(addPtr(headPtr, recoveryOffset));
            if ((recoveryOffset < entryCount) &&
                entries[recoveryIndex].valid &&
                !recoverYoungerMask_i[entries[recoveryIndex].robTag]) begin
