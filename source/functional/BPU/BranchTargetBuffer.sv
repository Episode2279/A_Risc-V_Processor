module BranchTargetBuffer import TypesPkg::*; #(
 ...
 input logic updateTaken_i, input instruction_addr_t updateTarget_i);
 ...
 logic [INDEX_W-1:0] qSet,qSet1,uSet; logic [TAG_W-1:0] qTag,qTag1,uTag;
 ...
 always_ff @(posedge clk or negedge rst) begin
  if(!rst) begin
   for(setIndex=0;setIndex<SETS;setIndex=setIndex+1) begin
    ...
    for(resetWay=0;resetWay<WAYS;resetWay=resetWay+1) begin validTable[setIndex][resetWay]=1'b0; tagTable[setIndex][resetWay]='0; targetTable[setIndex][resetWay]='0; end
   end
  end else if(updateValid_i&&updateTaken_i) begin
   validTable[uSet][selectedWay]<=1'b1; tagTable[uSet][selectedWay]<=uTag;
   targetTable[uSet][selectedWay]<=updateTarget_i; replaceWay[uSet]<=~selectedWay;
  end
 end
endmodule
