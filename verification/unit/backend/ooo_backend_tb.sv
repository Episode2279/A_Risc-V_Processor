...
module ooo_backend_tb;
    ...
    logic storeCommitObserved;
    logic loadCommitObserved;
    ...
    OoOBackend dut (
        ...
        .memoryWrite_o(memoryWrite), .memoryRequestId_o(memoryRequestId),
        ...
        .perfLsqOrderBlockedCycles_o(),
        .perfStoreBufferAliasBlockedCycles_o(),
        .perfMmioOrderBlockedCycles_o(),
        .perfDcacheRequestBlockedCycles_o(),
        .perfLsuInternalBlockedCycles_o(),
        .perfLsuFallbackCycles_o(),
    ...
    // One-cycle synchronous memory model used by the backend-only smoke test.
    // Stores complete on request acceptance; Loads return on the response
    // channel during the following cycle.
    always_ff @(posedge clk) begin
        if (!rst) begin
            ...
            if (memoryResponseValid && memoryResponseReady)
                memoryResponseValid <= 1'b0;
            if (memoryValid && !memoryWrite) begin
                memoryResponseValid <= 1'b1;
                memoryResponseData <= memoryReadData;
                memoryResponseId <= memoryRequestId;
            end
        end
    end
    ...
    initial begin
        ...
        storeCommitObserved = commitValid[0] && (commitPc[0] == 32'h118);
        loadCommitObserved = 1'b0;
        while (!memoryWrite) begin
            if (commitValid[0] && (commitPc[0] == 32'h118))
                storeCommitObserved = 1'b1;
            if ((commitValid[0] && (commitRd[0] == 7)) ||
                (commitValid[1] && (commitRd[1] == 7))) begin
                if ((commitValid[0] && (commitRd[0] == 7) &&
                     (commitData[0] != 32'hDEAD_BEEF)) ||
                    (commitValid[1] && (commitRd[1] == 7) &&
                     (commitData[1] != 32'hDEAD_BEEF)))
                    $fatal(1, "Store-forwarded Load committed wrong data");
                loadCommitObserved = 1'b1;
            end
            tick();
        end
        if (memoryAddress != 32'h30 || memoryWriteData != 32'hDEAD_BEEF)
            $fatal(1, "Store Buffer drain request is incorrect");
        if (commitValid[0] && (commitPc[0] == 32'h118))
            storeCommitObserved = 1'b1;
        if ((commitValid[0] && (commitRd[0] == 7)) ||
            (commitValid[1] && (commitRd[1] == 7))) begin
        ...
        if (!storeCommitObserved)
            $fatal(1, "Store did not retire before Store Buffer drain");
        if (!loadCommitObserved)
